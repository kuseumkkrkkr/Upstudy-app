"""
Exact Python replica of the production rating algorithm from omj/rating_service.py.

This module replicates:
  - rating_config.py   (CONFIG constants)
  - rating_storage.py  (in-memory DB operations)
  - rating_service.py  (compute_* helpers + apply_rating_update + fetch_user_rating)

Usage:
    from sim_data.rating_algorithm_replica import CONFIG, InMemoryRatingDB, apply_rating_update
    db = InMemoryRatingDB()
    result = apply_rating_update(db, user_id="u1", ...)
"""
...
# ---------------------------------------------------------------------------
# rating_config.py  (exact replica)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Helpers (exact replicas from rating_service.py)
# ---------------------------------------------------------------------------
...
_EXCLUDED_TAGS = {_normalize_tag("규칙연산")}
...
# ---------------------------------------------------------------------------
# In-memory DB (replaces SQLite rating_storage.py)
# ---------------------------------------------------------------------------
@dataclass
class UserRatingRow:
    user_id: str
    ...
    recent_count: int = 0
    recent_sum: int = 0
    tag_rating_sum: float = 0.0
    tag_rating_count: int = 0
    created_at: str = field(default_factory=lambda: _now_utc().isoformat(timespec="seconds") + "Z")
    updated_at: str = field(default_factory=lambda: _now_utc().isoformat(timespec="seconds") + "Z")
...
class TagStatRow:
    ...
    attempts: int = 0
    rating: float = CONFIG.DEFAULT_RATING
    rating_prev: float = CONFIG.DEFAULT_RATING
    updated_at: str = field(default_factory=lambda: _now_utc().isoformat(timespec="seconds") + "Z")


class InMemoryRatingDB:
    """In-memory replica of the SQLite rating tables."""

    def __init__(self):
        self.users: Dict[str, UserRatingRow] = {}
        self.tag_stats: Dict[Tuple[str, str], TagStatRow] = {}
        self.submissions: set = set()  # (user_id, submission_id)

    def get_user(self, user_id: str) -> Optional[UserRatingRow]:
        return self.users.get(user_id)

    def create_user(self, user_id: str, rating: float = CONFIG.DEFAULT_RATING) -> UserRatingRow:
    ...
    def get_tag_stats(self, user_id: str, tags: Iterable[str]) -> Dict[str, TagStatRow]:
        result: Dict[str, TagStatRow] = {}
        for tag in tags:
            key = (user_id, tag)
            if key in self.tag_stats:
                result[tag] = self.tag_stats[key]
        return result

    def list_tag_stats(self, user_id: str) -> List[TagStatRow]:
        return [row for (uid, _), row in self.tag_stats.items() if uid == user_id]

    def upsert_tag_stats(self, updates: List[Dict[str, Any]]) -> None:
        for item in updates:
            key = (item["user_id"], item["tag"])
            row = self.tag_stats.get(key)
            if row is None:
                row = TagStatRow(
                    user_id=item["user_id"],
                    tag=item["tag"],
                    rating=item["rating"],
                    rating_prev=item["rating_prev"],
                    attempts=item["attempts"],
                    updated_at=item["updated_at"],
                )
                self.tag_stats[key] = row
            else:
                row.rating = item["rating"]
                row.rating_prev = item["rating_prev"]
                row.attempts = item["attempts"]
                row.updated_at = item["updated_at"]

    def mark_submission(self, user_id: str, submission_id: str) -> bool:
...
# ---------------------------------------------------------------------------
# apply_rating_update (exact replica of rating_service.py logic)
# ---------------------------------------------------------------------------
def apply_rating_update(
    ...
    *,
    ...
    # --- submission dedup (optional) ---
    if submission_id:
        if not db.mark_submission(user_id, submission_id):
            user = db.get_user(user_id)
            if not user:
                user = db.create_user(user_id)
            recent_accuracy = (
                user.recent_sum / user.recent_count if user.recent_count > 0 else 0.0
            )
            return RatingResult(
                rating=user.rating,
                ovr=user.ovr,
    ...
    last_attempt_at = _parse_iso(user.last_attempt_at)
    recent_results = list(user.recent_results)
    recent_index = int(user.recent_index)
    recent_count = int(user.recent_count)
    recent_sum = int(user.recent_sum)

    tag_stats = db.get_tag_stats(user_id, normalized_tags)
    ...
    tag_rating_sum = float(user.tag_rating_sum)
    tag_rating_count = int(user.tag_rating_count)
    ...
    if normalized_tags and valid_flow_count == 0:
        ...
        delta_user = sum(
            (item["rating"] - (tag_stats.get(item["tag"], TagStatRow(user_id=user_id, tag=item["tag"])).rating))
            * per
    ...
    db.update_user(
        ...
        tag_rating_sum=tag_rating_sum,
        tag_rating_count=tag_rating_count,
        updated_at=now_iso,
    )

    db.upsert_tag_stats(tag_updates)
...
def fetch_user_rating(db: InMemoryRatingDB, user_id: str) -> RatingResult:
    user = db.get_user(user_id)
    if not user:
        user = db.create_user(user_id)
    recent_accuracy = (
