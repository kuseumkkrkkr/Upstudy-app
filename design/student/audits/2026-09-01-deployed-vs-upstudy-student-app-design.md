...
- 재확인 시 Production 환경변수 목록은 `STUDENT_STORE_DEMO`만 유지되고 필수 데이터 계층 키는 추가되지 않았다. 외부 설정 변경 없이는 readiness 재검증을 진행할 수 없다.
