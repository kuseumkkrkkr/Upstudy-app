...
class _TeacherTextbookReaderPageState extends State<TeacherTextbookReaderPage> {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              runtime['passed'] == true
                  ? '?¥Ïàò Ï°∞Í±¥ Ï∂©Ï°±: ${_completionPercent(_completion)}'
                  : '?¥ÏàòÎ•? ${_completionPercent(_completion)} (?´Í∏∞)',
            ),
          ),
        );
      }
  ...
  void _openWordSheet(String word) {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _ReaderAuxPanel(
        title: word,
        content: definition ?? '?ÑÏû¨ ÍµêÏû¨?êÎäî ???®Ïñ¥???¨Ï†Ñ ?ïÏùòÍ∞Ä ?±Î°ù???àÏ? ?äÏäµ?àÎã§.',
      ),
    );
  }
  ...
  Future<void> _openLink(String rawUrl) async {
    ...
    if (!await launchUrl(uri, mode: LaunchMode.externalApplication)) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('ÎßÅÌÅ¨Î•??????ÜÏäµ?àÎã§.')),
      );
    }
  }
  ...
  List<_TextbookSectionPage> _flattenBookPages(BookData book) {
    ...
    for (final chapter in book.chapters) {
      final introTitle = '${chapter.title} (Í∞úÏöî)';
      result.add(_TextbookSectionPage(
        index: index++,
        title: introTitle,
        body: chapter.intro.isEmpty
            ? '?ÑÏßÅ Î≥∏Î¨∏??ÎπÑÏñ¥ ?àÏäµ?àÎã§.'
            : chapter.intro.join('\n'),
    ...
    if (result.isEmpty) {
      result.add(
        _TextbookSectionPage(
          index: 1,
          title: book.title,
          body: book.subtitle.isNotEmpty ? book.subtitle : '?òÏù¥ÏßÄÍ∞Ä ÎπÑÏñ¥ ?àÏäµ?àÎã§.',
  ...
  String _buildNumberLabel(int index) {
    ...
    return '?òÏù¥ÏßÄ $index ¬∑ Î™®Îìà Î≤îÏúÑ: $_runtimePageFrom~$_runtimePageTo';
  }
                      ...
                      if (value == 'toc') {
                        ...
                        showModalBottomSheet(
                          ...
                          builder: (_) => _ReaderAuxPanel(
                            title: 'ÏßÑÌñâ Ï°∞Í±¥',
                            content:
                                'Î≤îÏúÑ: $_runtimePageFrom ~ $_runtimePageTo\n'
                                'ÏµúÏÜå?ôÏäµ?úÍ∞Ñ: ${_minMinutes <= 0 ? '?ÜÏùå' : '$_minMinutesÎ∂?}\n'
                                'ÏßÑÌñâÎ•? ${_completionPercent(_completion)}',
                          ),
                        );
                      }
                    },
...
class _ProgressHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final timeLabel = '$elapsedSecondsÏ¥?/ ${minSeconds == 0 ? '?úÌïú ?ÜÏùå' : '${minSeconds}Ï¥?}';
          ...
          Wrap(
            ...
            children: [
              _MiniBadge(
                ...
                text: '?ÑÏû¨ ${max(from, min(to, currentPage))} / $to',
              ),
              _MiniBadge(
                icon: Icons.timeline,
                text: 'Î≤îÏúÑ ÏßÑÌñâ ${((pageRatio).clamp(0.0, 1.0) * 100).round()}%',
              ),
              _MiniBadge(
                icon: Icons.timer,
                text: '?¥Îûå?úÍ∞Ñ $timeLabel',
              ),
              if (enforceMinMinutes && minMinutes > 0)
                _MiniBadge(
                  icon: Icons.lock_clock,
                  text: 'ÏµúÏÜå?úÍ∞Ñ ${minMinutes}Î∂?,
                ),
            ],
          ),
        ],
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text('ÏµúÏÜå ?úÍ∞Ñ: ${minMinutes <= 0 ? '?ÜÏùå' : '$minMinutesÎ∂?}'),
            Text('Í∞ïÏ†ú ?ÅÏö©: ${enforceMinMinutes ? '?? : '?ÑÎãà??}'),
            Text('?¥Ïàò?? $completionPercent'),
            const SizedBox(height: 16),
            ...
            Expanded(
              child: links.isEmpty
                  ? const Text(
                      '?ÑÏû¨ ?òÏù¥ÏßÄ??ÎßÅÌÅ¨Í∞Ä ?ÜÏäµ?àÎã§.',
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Column(
        children: [
          const SizedBox(height: 8),
          const Text('Î™©Ï∞®', style: TextStyle(fontWeight: FontWeight.w700)),
          const Divider(),
          Expanded(
            child: ListView.separated(
              itemCount: pages.length,
...
const _wordGlossary = <String, String>{
  '?®Ïàò': '?ÖÎ†• Í∞íÏùÑ Î∞õÏïÑ Ï∂úÎ†• Í∞íÏùÑ Í≥ÑÏÇ∞???¥Îäî ?òÌïô??Í∑úÏπô ?êÎäî ?ÑÎ°úÍ∑∏Îû® Íµ¨ÏÑ± ?îÏÜå?ÖÎãà??',
  'Î≥Ä??: 'Í∞íÏùÑ ?Ä?•ÌïòÍ≥??¥Î¶Ñ?ºÎ°ú ?∏Ï∂ú?òÎäî ?òÌïô/?ÑÎ°úÍ∑∏ÎûòÎ∞çÏùò Í∏∞Ï¥à ?®ÏúÑ?ÖÎãà??',
  'ÎØ∏Î∂Ñ': 'Î≥Ä?îÏú®??Í≥ÑÏÇ∞?òÎäî ÎØ∏Ï†ÅÎ∂??∞ÏÇ∞?ºÎ°ú ?úÍ∞Ñ Î≥Ä?îÏùò Í∏∞Ïö∏Í∏∞Î? ?òÌ??ÖÎãà??',
  '?ÅÎ∂Ñ': 'Î©¥Ï†Å ?êÎäî ?ÑÏ†Å?âÏùÑ ?òÌ??¥Îäî Í≥ÑÏÇ∞?ºÎ°ú, ?®Ïàò??ÎØ∏Î∂Ñ??Í±∞Íæ∏Î°??∏Îäî ?∞ÏÇ∞?ÖÎãà??',
  '?ïÏùò': '?©Ïñ¥???ïÌôï???òÎ??Ä Î≤îÏúÑÎ•??òÌ??¥Îäî Í∏∞Ïà†???§Î™Ö?ÖÎãà??',
  'ÍµêÏû¨': '?ôÏäµ???¥Ïö©???ïÎ¶¨????Íµ¨Ï°∞?îÎêú ?çÏä§??Íµ¨ÏÑ± Î¨∏ÏÑú?ÖÎãà??',
};
