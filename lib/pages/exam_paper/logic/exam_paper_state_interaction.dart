...
mixin _ExamPaperInteractionMixin on _ExamPaperStateBase {
  ...
  void _scrollToPage(int index) {
    ...
    final minY = viewport.height - contentHeight - _scrollEdgePadding;
    final maxY = _scrollEdgePadding;
    final targetY = (-pageTop + _scrollEdgePadding).clamp(minY, maxY);
    final centerX = (viewport.width - _paperWidth * scale) / 2;
    _panOffset = Offset(centerX, targetY);
    _updateViewMatrix();
  }
  ...
  bool _isPanPointer(PointerEvent event) {
    return _scrollEnabled || event.kind == PointerDeviceKind.touch;
  }
  ...
  void _handlePointerMove(PointerMoveEvent event) {
    ...
    if (_isPanPointer(event)) {
      _handleScrollDelta(-event.delta.dy, allowWhenDisabled: !_scrollEnabled);
      return;
    }
