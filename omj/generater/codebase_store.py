...
def record_seed_attempt(codebase_id: int, code_hash: str, success: bool) -> None:
    ...
    with _get_db_connection() as conn:
        ...
        if row:
            ...
            conn.execute(
                """
                UPDATE codebase_seed_stats
                SET attempts = ?, successes = ?, updated_at = CURRENT_TIMESTAMP
                WHERE codebase_id = ? AND code_hash = ?
                """,
                (attempts, successes, codebase_id, code_hash),
            )
        else:
            conn.execute(
                """
                ...
                VALUES (?, ?, ?, ?)
                """,
                (codebase_id, code_hash, 1, 1 if success else 0),
            )
        conn.commit()
...
def list_codebase_stats(*, auto_delete_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    for entry in codebases:
        ...
        if entry_id is not None:
            ...
            cached_count = count_cached_seeds(entry_id, code_hash)
