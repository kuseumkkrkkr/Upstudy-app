...
채점 원칙(중요):
1) OCR 결과는 필기 인식 과정에서 숫자·부호·공백이 일부 틀릴 수 있는 보조 근거다.
2) OCR_ALL_FORMULAS가 풀이 흐름·문제 정답과 자연스럽게 일치하면, 사소한 OCR 표기 오차(공백, ×/x, 괄호 모양, 한 자리 부호 누락)는 학생에게 불리하게 해석하지 말고 "O"로 판단할 수 있다.
3) 다만 서로 다른 수식, 핵심 숫자·연산의 불일치, 중간 단계 누락·논리 비약은 "X"다. 이미지에 없는 새로운 풀이를 만들어 내지는 않는다.
4) 최종 답이 QUEST_ANSWER와 명확히 일치하고 과정이 대체로 뒷받침되면 OCR의 경미한 오류만으로 오답 처리하지 않는다.
4) 정답만 맞고 과정이 전혀 확인되지 않으면 "X".
5) 각 flow는 독립적으로 판단하되, 다음 단계가 성립하려면 이전 단계가 대체로 확인되어야 한다.
