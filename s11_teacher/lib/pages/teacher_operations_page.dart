  ...
  Widget build(BuildContext context) {
    return Ios26FrostedCard(
      ...
      child: Wrap(
        ...
        children: [
          SegmentedButton<_OpsSection>(
            segments: const [
              ButtonSegment(
                value: _OpsSection.finance,
                icon: Icon(Icons.account_balance_wallet_rounded),
                label: Text('재무제표'),
              ),
              ButtonSegment(
                value: _OpsSection.schedule,
          ...
          if (section == _OpsSection.schedule)
            SegmentedButton<_ScheduleScope>(
              segments: const [
                ButtonSegment(value: _ScheduleScope.day, label: Text('일간')),
                ButtonSegment(value: _ScheduleScope.month, label: Text('월간')),
              ],
              selected: {scheduleScope},
              onSelectionChanged: (value) =>
                  onScheduleScopeChanged(value.first),
            ),
          IconButton.filledTonal(
            tooltip: '이전',
            icon: const Icon(Icons.chevron_left_rounded),
            onPressed: () => onMoveDate(-1),
          ),
          OutlinedButton.icon(
            icon: const Icon(Icons.event_rounded),
            label: Text(_dateLabel(selectedDate, scheduleScope, section)),
              ...
              Wrap(
                ...
                children: [
                  ...
                  OutlinedButton.icon(
                    ...
                    label: Text(_shortDate(occurredOn)),
                    onPressed: onPickDate,
                  ),
                  _FieldBox(
                    width: 240,
